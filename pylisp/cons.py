...
def cons_parcel(
    ...
    a_bound: hv.Knot = hv.bind(a, env.lhs)
    b_bound: hv.Knot = hv.bind(b, env.rhs)
...
def cons(env: LispEnv, a: HyperBinary, b: HyperBinary) -> hv.Sparkle:
    ...
    id: hv.Sparkle = env.fresh_id()
    seed = hv.Seed128(id.domain().id(), id.pod().seed())
    cell: hv.Parcel = cons_parcel(env, seed, a, b)
...
def car(env: LispEnv, id: hv.Sparkle) -> HyperBinary:
    ...
    cell: HyperBinary = _lookup_cell(env, id)
    released: HyperBinary = hv.release(cell, env.lhs)
...
def _lookup_cell(env: LispEnv, id: hv.Sparkle) -> HyperBinary:
    ...
    cell: HyperBinary | None = env.substrate.get_code(id)
    if cell is not None:
        return cell
