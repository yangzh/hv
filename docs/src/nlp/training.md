...
Looking back, hierarchical inference over such a state space is a generic algorithm, applicable well beyond linguistic parsing — anywhere the potential state space is too immense to tabulate. A robust training/inference pipeline that accommodates vast and rich state space is a valuable by-product of this project in its own right. 
...
**BEGIN is not trained** as a transition source. "How do chains open?" has enormous fan-out at fine granularity (thousands of distinct edges). Instead, chains open through the *down* family — the first child's edge is the parent's climb anchor, while the sentence root opens under a uniform prior. BEGIN exists as a conceptual marker, not a statistic.
...
- [LearnerPool](../concepts/learner_pool.md): accommodating high-dynamic-range of learners
