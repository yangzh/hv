...
A Dart encodes a directed link from a source `A` to a destination `B`. Given the Dart and either endpoint, the other endpoint can be recovered: `P ⊗ B` recovers `A`, and `A ⊗ P^{-1}` recovers `B`.

Dart is the structured wrapper for the [release](../api/hv/operators.md#release) operation — `Release(A, B)` returns a Dart with `A` as source and `B` as destination.
...
| **Dart** | Bind A with Inv(B) | Directional | One-directional references |
