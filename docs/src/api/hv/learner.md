...
learner.revitalize(1)             # reset age: speed up future learning
```
{{#endtab}}
{{#tab name="Go"}}
```go
...
learner.BundleSet(set)            // every member, sharing one budget
learner.Merge(other)              // absorb another learner
learner.Revitalize(1)             // reset age: speed up future learning
```
{{#endtab}}
{{#tab name="Rust"}}
...
learner.bundle_set(&set)?;        // every member, sharing one budget
learner.merge(&other)?;           // absorb another learner
learner.revitalize(1);            // reset age: speed up future learning
```
{{#endtab}}
...
learner.DiversityMargin()    // uint32: recovery margin of the last write
...
learner.diversity_margin()   // u32: recovery margin of the last write
